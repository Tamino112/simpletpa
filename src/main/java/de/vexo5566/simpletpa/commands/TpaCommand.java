...
public class TpaCommand implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        TPAManager.TpaRequest request = new TPAManager.TpaRequest(player, target, false);
        plugin.getTpaManager().addRequest(request);

        player.sendMessage(plugin.msg("messages.tpa_sent").replace("%target%", target.getName()));

        target.sendMessage(plugin.msg("messages.tpa_received").replace("%sender%", player.getName()));
